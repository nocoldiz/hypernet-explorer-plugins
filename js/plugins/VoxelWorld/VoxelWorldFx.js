...
(() => {
    ...
    class UnderwaterFx {
        ...
        update(camX, camY, camZ, delta) {
            ...
            // At the buffer rather than through the accessors, the way WheelFx
            // walks its own: six hundred bubbles was two thousand four hundred
            // accessor calls a frame to lift them.
            const a = pos.array, n = pos.count;
            for (let i = 0; i < n; i++) {
                const o = i * 3;
                let y = a[o + 1] + 28 * delta;
                if (y > 150) {
                    y = -150;
                    a[o]     = (Math.random() - 0.5) * 800;
                    a[o + 2] = (Math.random() - 0.5) * 800;
                }
                a[o + 1] = y;
            }
    ...
    class SkyFx {
        ...
        _moonTonight() {
            ...
            // The moon turns once a night, not sixty times a second. This used
            // to walk the calendar, run the phase maths and hand back a fresh
            // object on every frame, inside a try/catch, for an answer that
            // cannot change until the date does. _paintMoon already buckets its
            // repaint; this is what feeds it.
            const day = (typeof SR.getGameDate === 'function')
                ? String(SR.getGameDate()) : '';
            if (this._moonDay === day && this._moonAns) return this._moonAns;
