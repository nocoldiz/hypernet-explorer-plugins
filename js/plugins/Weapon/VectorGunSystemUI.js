...
(() => {
  ...
  class Scene_VectorGun extends Scene_MenuBase {
    ...
    /** Walks one card within the current grid line without wrapping across lines or sections. */
    moveCursor(step) {
      const lines = this.lines();
      let at = -1;
      let col = 0;
      ...
      if (nextCol >= 0 && nextCol < lines[at].length) {
        this.setIndex(lines[at][nextCol]);
      }
    }
    ...
    _statusHTML() {
      ...
      const shut = VG.lockedModes ? VG.lockedModes() : [];
      if (shut.length) {
    ...
    _modeCard(key) {
      ...
      const locked = VG.isModeUnlocked ? !VG.isModeUnlocked(key) : false;
      return {
