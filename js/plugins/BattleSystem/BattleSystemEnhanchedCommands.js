...
(() => {
  ...
  // Backing out of a target lands on the command list and nowhere else. The
  // base Scene_Battle answers the same cancel by reopening whichever of the
  // engine's own skill or item windows the command symbol names, and in this
  // battle those two windows are vestigial: every list is rows in the command
  // menu now (CategorizedBattleSkills.js, ItemSystemInventory.js). Left to
  // stand, the engine's bare skill list opened over the menu and read as the
  // ordinary out-of-battle Skills screen.
  Scene_Battle.prototype._bseStandDownLegacyLists = function () {
    for (const win of [this._skillWindow, this._itemWindow]) {
      if (win) { win.deactivate(); win.hide(); }
    }
  };
